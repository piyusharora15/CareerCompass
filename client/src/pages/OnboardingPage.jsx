    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            await axios.post('http://localhost:5000/users/profile', 
                { industry, currentRole, desiredRole, skills: skills.split(',').map(s => s.trim()) }, // We format the skills string into an array.
    ...
    return (
        <div className="min-h-screen bg-gray-900 text-white flex items-center justify-center p-4">
            <div className="w-full max-w-2xl bg-gray-800 p-8 rounded-lg shadow-lg">
                <h1 className="text-3xl font-bold text-center mb-2">Welcome to LakshyaAI!</h1>
                <p className="text-center text-gray-400 mb-8">Let's set up your career profile to get started.</p>
                ...
                <form onSubmit={handleSubmit} className="space-y-6">
                    ...
                    <div>
                        <label htmlFor="industry" className="block text-sm font-medium text-gray-300 mb-2">What industry are you in?</label>
                        <input type="text" id="industry" value={industry} onChange={(e) => setIndustry(e.target.value)} required className="w-full bg-gray-700 rounded-md p-3" placeholder="e.g., Software Development"/>
                    </div>
                    {/* ... other form fields ... */}
                     <div>
                        <label htmlFor="skills" className="block text-sm font-medium text-gray-300 mb-2">What are your current skills? (comma-separated)</label>
                        <textarea id="skills" value={skills} onChange={(e) => setSkills(e.target.value)} required rows="3" className="w-full bg-gray-700 rounded-md p-3" placeholder="e.g., React, Node.js, SQL, Agile"></textarea>
                    </div>
